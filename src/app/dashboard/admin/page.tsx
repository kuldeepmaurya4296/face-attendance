...
import { Users, CheckSquare, FileText, Loader2 } from 'lucide-react';
...
export default function AdminOverview() {
  ...
  return (
    <div className="p-4 md:p-8 space-y-6">
      <PageHeader title="Admin Dashboard" subtitle="Operations Overview" />
      {loading ? <div className="flex justify-center p-8"><Loader2 className="animate-spin text-primary" /></div> : (
        <div className="border border-border rounded-lg bg-white p-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <StatCard label="Personnel" value={stats.users} icon={Users} colorClass="text-primary" />
            <StatCard label="Attendance Logs" value={stats.attendance} icon={CheckSquare} colorClass="text-success" />
            <StatCard label="Leaves" value={stats.leaves} icon={FileText} colorClass="text-warning" />
          </div>
        </div>
      )}
    </div>
